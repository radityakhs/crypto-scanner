...
export async function runLearning(chatId, windowArg = '12h') {
  ...
  const summary = await summarizeLearningWindow(windowMs);
  const { lessons, raw } = await generateLessons(summary);
  const runId = await storeLearningRun(windowMs, summary, lessons, raw);
...
export async function sendLessons(chatId) {
  const rows = await dbAll(
    `SELECT TOP 10 id, created_at_ms, lesson FROM learning_lessons WHERE status='active' ORDER BY id DESC`,
  );
