...
export async function consumeNumericFilterInput(chatId, text, userMessageId = null) {
  ...
  if (pending.type === 'strategy') {
    ...
    if (pending.messageId) {
      await bot.editMessageText(await strategyMenuText(), {
        chat_id: chatId,
        message_id: pending.messageId,
        parse_mode: 'HTML',
        disable_web_page_preview: true,
        ...await strategyKeyboard(),
      }).catch(() => strategyMenuText().then(t => strategyKeyboard().then(k => bot.sendMessage(chatId, t, { parse_mode: 'HTML', ...k }))));
    } else {
      await strategyMenuText().then(t => strategyKeyboard().then(k => bot.sendMessage(chatId, t, { parse_mode: 'HTML', ...k })));
    }
  } else {
    await setSetting(pending.key, String(value));
    if (pending.messageId) {
      await bot.editMessageText(await filtersText(), {
        chat_id: chatId,
        message_id: pending.messageId,
        parse_mode: 'HTML',
        disable_web_page_preview: true,
        ...filtersKeyboard(),
      }).catch(() => filtersText().then(t => bot.sendMessage(chatId, t, { parse_mode: 'HTML', ...filtersKeyboard() })));
    } else {
      await bot.sendMessage(chatId, await filtersText(), { parse_mode: 'HTML', ...filtersKeyboard() });
    }
  }
  return true;
}
