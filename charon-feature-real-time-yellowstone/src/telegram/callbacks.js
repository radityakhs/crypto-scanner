...
export async function handleCallback(query) {
  ...
  if (data === 'menu:agent') {
    return editMenuMessage(query, await agentText(), await agentKeyboard());
  }
  if (data === 'toggle:agent') {
    await setSetting('agent_enabled', await boolSetting('agent_enabled', true) ? 'false' : 'true');
    return editMenuMessage(query, await agentText(), await agentKeyboard());
  }
  if (data === 'toggle:trending_enabled' || data === 'toggle:trending_allow_degen') {
    const key = data.replace('toggle:', '');
    await setSetting(key, await boolSetting(key, key === 'trending_enabled') ? 'false' : 'true');
    return editMenuMessage(query, await filtersText(), filtersKeyboard());
  }
  if (data === 'menu:filters') return editMenuMessage(query, await filtersText(), filtersKeyboard());
  if (data === 'menu:strategy') return editMenuMessage(query, await strategyMenuText(), await strategyKeyboard());
  if (data === 'menu:wallets') return editMenuMessage(query, await walletsText(), navKeyboard());
  ...
  if (data === 'menu:settings') return editMenuMessage(query, `${await agentText()}\n\n${await filtersText()}`, navKeyboard([
    [
  ...
  if (data.startsWith('strategy:select:')) {
    ...
    return editMenuMessage(query, await strategyMenuText(), await strategyKeyboard());
  }
  ...
  if (kind === 'ign') {
    await updateCandidateStatus(Number(id), 'ignored');
    return bot.sendMessage(chatId, 'Ignored candidate.');
  }
  if (kind === 'buy') {
    const row = await candidateById(Number(id));
    if (!row) return bot.sendMessage(chatId, 'Candidate not found.');
    if (!await canOpenMorePositions()) {
      return bot.sendMessage(chatId, `Max open positions reached (${await openPositionCount()}/${await numSetting('max_open_positions', 3)}). Close one first or raise the limit.`);
    }
    const candidate = row.candidate;
    const decision = { verdict: 'BUY', confidence: 100, reason: 'Manual dry buy', risks: [], suggested_tp_percent: await numSetting('default_tp_percent', 50), suggested_sl_percent: await numSetting('default_sl_percent', -25) };
    const decisionId = await storeDecision(row.id, candidate, decision);
    decision.id = decisionId;
    if (await tradingMode() === 'live') {
    ...
    await logDecisionEvent({ batchId: 'manual', triggerCandidateId: row.id, selectedRow: row, rows: [row], decision, mode: await tradingMode(), action: 'manual_dry_run_entry', execution: { positionId } });
...
async function handleStratConfig(query, chatId, key) {
  ...
  if (boolKeys.has(key)) {
    ...
    await updateStrategyConfig(strat.id, newConfig);
    return editMenuMessage(query, await strategyMenuText(), await strategyKeyboard());
  }
...
async function updateSettingFromButton(query, key, value) {
  ...
  const isAgent = key.startsWith('default_') || ['dry_run_buy_sol','trading_mode','llm_min_confidence','llm_candidate_pick_count','llm_candidate_max_age_ms','max_open_positions'].includes(key);
  const text = isAgent ? await agentText() : await filtersText();
  const extra = isAgent ? await agentKeyboard() : filtersKeyboard();
