...
export async function sendCandidateAlert(candidateId, candidate, decision) {
  ...
  await dbRun(
    `INSERT INTO alerts (candidate_id,mint,kind,sent_at_ms,telegram_message_id,payload_json) VALUES (@p0,@p1,@p2,@p3,@p4,@p5)`,
    [candidateId, candidate.token.mint, 'candidate', now(), sent.message_id, json({ candidate, decision })],
  );
}

export async function sendBatchReveal(batchId, rows, decision, triggerCandidateId) {
  const sent = await sendTelegram(batchRevealSummary(batchId, rows, decision, triggerCandidateId), batchRevealButtons(batchId, rows, decision, triggerCandidateId));
  await dbRun(
    `INSERT INTO alerts (candidate_id,mint,kind,sent_at_ms,telegram_message_id,payload_json) VALUES (@p0,@p1,@p2,@p3,@p4,@p5)`,
    [triggerCandidateId || null, decision.selected_mint || rows.find(r => r.id === Number(triggerCandidateId))?.candidate?.token?.mint || 'batch', 'batch_reveal', now(), sent.message_id, json({ batchId, candidateIds: rows.map(r => r.id), decision, triggerCandidateId })],
  );
}
...
export async function sendBatch(chatId, batchId) {
  ...
  const lines = ['🧭 <b>Screening Batch</b>', '', `Batch: <b>#${batchId}</b> · Decision: <b>${escapeHtml(batch.verdict)}</b> ${fmtPct(batch.confidence)}`, batch.reason ? `Reason: ${escapeHtml(String(batch.reason).slice(0, 500))}` : null, '', ...batch.rows.map((row, index) => compactCandidateLine(row, index + 1))];
  const keyboard = batch.rows.slice(0, 10).map((row, index) => ([{ text: `${index + 1}. ${row.candidate.token?.symbol || short(row.candidate.token?.mint || '')}`, callback_data: `cand:${row.id}` }]));
...
export async function sendPositionOpen(positionId) {
  const position = await dbGet('SELECT * FROM dry_run_positions WHERE id=@p0', [positionId]);
...
export async function sendTradeIntent(intentId, candidate, decision) {
  await sendTelegram([
    ...
    `Size: <b>${fmtSol(await numSetting('dry_run_buy_sol', 0.1))} SOL</b>`,
