...
export async function filtersText() {
  ...
  return [
    ...
    '',
    `Trending: <b>${await boolSetting('trending_enabled', true) ? 'on' : 'off'}</b> · Source: <b>${escapeHtml(await setting('trending_source', 'jupiter'))}</b>`,
    `GMGN status: token-info ${escapeHtml(gmgnStatusText('token'))} · trending ${escapeHtml(gmgnStatusText('trending'))}`,
    `Trending interval: ${escapeHtml(await setting('trending_interval', '5m'))} · Limit: ${await numSetting('trending_limit', 100)}`,
...
export async function agentText() {
  ...
  return [
    ...
    `Agent: <b>${await boolSetting('agent_enabled', true) ? 'on' : 'off'}</b>`,
    `Mode: <b>${escapeHtml(await tradingMode())}</b>`,
    `LLM: <b>${strat.use_llm && ENABLE_LLM && LLM_API_KEY ? 'configured' : 'disabled'}</b>`,
    `Confidence: ${fmtPct(strat.llm_min_confidence || await numSetting('llm_min_confidence', 75))}`,
    `Open positions: ${await openPositionCount()}/${strat.max_open_positions || 'unlimited'}`,
    `Batch candidates: ${await numSetting('llm_candidate_pick_count', 10)}`,
    `Candidate freshness: ${Math.round(await numSetting('llm_candidate_max_age_ms', 600000) / 1000)}s`,
...
export async function walletsText() {
  const rows = await savedWallets();
...
export async function positionsText() {
  const rows = await allPositions(12);
  const text = rows.length ? rows.map(formatPosition).join('\n\n') : 'No dry-run positions yet.';
  return `📍 <b>Positions</b>\n\n${text}`;
}

export async function strategyMenuText() {
  const strat = await activeStrategy();
  const all = await allStrategies();
...
export async function strategyKeyboard() {
  const strat = await activeStrategy();
  const all = await allStrategies();
